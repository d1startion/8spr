...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	// Проверяем ошибки курсора после цикла
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("адрес можно изменить только для посылок со статусом registered")
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("удалить можно только посылки со статусом registered")
	}
